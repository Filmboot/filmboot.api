...
class LoadCategoriesCommand extends ContainerAwareCommand
{
    ...
    {
        $this
            ->setName('filmbot:award:load:category')
            ->setDescription('Loads category from yml file')
            ->addArgument('file', InputArgument::REQUIRED, 'Path of file to be loaded')
            ->setHelp(
                'The <info>filmbot:award:load:category</info> command loads content of file passed by argument 
<info>php app/console filmbot:award:load:category <path-of-file></info>'
            );
    }
    ...
    public function loadCategories($path)
    {
        ...
        foreach ($fixtures as $values) {
            $award = $container->get('filmbot_award.manager.category')->create();
