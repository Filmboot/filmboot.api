...
namespace Filmboot\ArtistBundle\Command;
 ...
 *
 * @package Filmboot\ArtistBundle\Command
 */
class LoadArtistsCommand extends ContainerAwareCommand
{
    ...
    {
        $this
            ->setName('filmboot:artist:load:artist')
            ->setDescription('Loads artist from yml file')
            ->addArgument('file', InputArgument::REQUIRED, 'Path of file to be loaded')
            ->setHelp(
                'The <info>filmboot:artist:load:artist</info> command loads content of file passed by argument 
<info>php app/console filmboot:artist:load:artist <path-of-file></info>'
            );
    }
    ...
    public function loadArtists($path)
    {
        ...
        foreach ($fixtures as $values) {
            $artist = $container->get('filmboot_artist.manager.artist')->create();
